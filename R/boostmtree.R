...
##' base learners (Pande et al., 2017).  Covariate-time interactions are
##' modeled using penalized B-splines (P-splines) with estimated adaptive
##' smoothing parameter.
##' 
##' 
...
##' \emph{phi}. Multivariate trees are used for base learners and weighted
##' least squares is used for solving the terminal node optimization problem.
##' This provides solutions to the core parameters of the algorithm.  For
##' ancillary parameters, a mixed-model formulation is used to estimate the
##' smoothing parameter associated with the B-splines used for the
##' time-interaction effect, although the user can manually set the smoothing
##' parameter as well.  Ancillary parameters \emph{rho} and \emph{phi} are
##' estimated using GLS (generalized least squares).
##' 
##' In the original boostmtree algorithm (Pande et al., 2017), the
##' equicorrelation parameter \emph{rho} is used in two places in the
##' algorithm: (1) for growing trees using the gradient, which depends upon
##' \emph{rho}; and (2) for solving the terminal node optimization problem
##' which also uses the gradient.  However, Pande (2017) observed that setting
##' \emph{rho} to zero in the gradient used for growing trees improved
##' performance of the algorithm, especially in high dimensions.  For this
##' reason the default setting used in this algorithm is to set \emph{rho} to
##' zero in the gradient for (1).  The \code{rho} in the gradient for (2) is
##' not touched.  The option \code{mod.grad} specifies whether a modified
##' gradient is used in the tree growing process and is TRUE by default.
##' 
...
##' performance and has the added advantage that out-of-sample data
...
##' alternatively, initialize the environment variable \command{MC_CORES} in
...
##' only high CPU usage but also high RAM usage, especially when using
...
##' {"Continuous", "Binary","Nominal","Ordinal"} based on the scale of
##' \code{y}.
...
##' @param mtry Number of \code{x} variables selected randomly for tree
##' fitting. Default is use all \code{x} variables.
...
##' when d < 1).
##' @param pen.ord Differencing order used to define the penalty with
...
##' @param forest.tol Tolerance used for forest weighted least squares
...
##' components:
##' 
##' \item{x}{The x-values, but with only one row per individual (i.e.
##' duplicated rows are removed). Values sorted on \code{id}.}
##' \item{xvar.names}{X-variable names.} \item{time}{List with each component
##' containing the time points for a given individual. Values sorted on
##' \code{id}.} \item{id}{Sorted subject identifier.} \item{y}{List with each
##' component containing the observed y-values for a given individual. Values
##' sorted on \code{id}.} \item{Yorg}{For family == "Nominal" or family ==
##' "Ordinal", this provides the response in list-format where each element
##' coverted the response into the binary response.} \item{family}{Family of
##' \code{y}.} \item{ymean}{Overall mean of y-values for all individuals. If
##' \code{family} = "Binary", \code{ymean} = 0.} \item{ysd}{Overall standard
##' deviation of y-values for all individuals. If \code{family} = "Binary",
##' \code{ysd} = 1.} \item{na.action}{Remove missing values or impute?}
...
##' at the optimal number of iterations \code{Mopt}.  If the family ==
##' "Nominal" or family == "Ordinal", \code{mu} will have a higher level of
##' list to accommodate binary responses generated from nominal or ordinal
##' response.} \item{Prob_class}{For family == "Ordinal", this provides
##' individual probabilty rather than cumulative probabilty.}
##' \item{lambda}{Smoothing parameter. Results provided in vector or matrix
##' form, depending on whether family == c("Continuous","Binary") or family ==
##' c("Nominal", "Ordinal").} \item{phi}{Variance parameter.Results provided in
##' vector or matrix form, depending on whether family ==
##' c("Continuous","Binary") or family == c("Nominal", "Ordinal").}
##' \item{rho}{Correlation parameter.Results provided in vector or matrix form,
##' depending on whether family == c("Continuous","Binary") or family ==
##' c("Nominal", "Ordinal").} \item{baselearner}{List of length \emph{M}
##' containing the base learners.} \item{membership}{List of length \emph{M},
##' with each component containing the terminal node membership for a given
##' boosting iteration.} \item{X.tm}{Design matrix for all the unique time
##' points.} \item{D}{Design matrix for each subject.} \item{d}{Degree of the
##' piecewise B-spline polynomial.} \item{pen.ord}{Penalization difference
##' order.} \item{K}{Number of terminal nodes.} \item{M}{Number of boosting
##' iterations.} \item{nu}{Boosting regularization parameter.}
##' \item{ntree}{Number of trees.} \item{cv.flag}{Whether in-sample CV is used
##' or not?} \item{err.rate}{In-sample standardized estimate of l1-error and
##' RMSE.} \item{rmse}{In-sample standardized RMSE at optimized \code{M}.}
...
##' data, \emph{Machine Learning}, 106(2): 277--305.
##' 
