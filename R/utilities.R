...
RemoveMiss.Fun <- function(X) {
  ...
  return(list(
    X = X,
    id.remove = if (length(WhichRow.remove) > 0)
      WhichRow.remove
    else
      NULL
  ))
...
l1Dist <- function(y1, y2) {
  ...
  mean(unlist(lapply(seq_along(y1), function(i) {
...
plot.profile.prx <- function(obj,
  ...
  muGrid <- lapply(seq_len(ncol(DbetaT)), function(i) {
    DbetaT[, i]
  })
  mu <- obj$boost.obj$mu
  time.hat <- obj$time
  muhat <- obj$muhat
  if (is.null(rnd.case)) {
    rnd.case <- sample(seq_len(nrow(prx)), size = 1)
  }
  ...
  prx.mu <- c(do.call(cbind, lapply(rnd.match, function(i) {
    muGrid[[i]]
  }))
  %*% rnd.prx / sum(rnd.prx))
  prx.which.time <- is.element(time.unq, unlist(lapply(rnd.match, function(i) {
    time[[i]]
  })))
  prx.time <- time.unq[prx.which.time]
  prx.mu <- prx.mu[prx.which.time]
  if (is.null(col))
    col <- rep(1, length(rnd.prx))
  plot(
    supsmu(rnd.time, rnd.mean),
    xlim = if (restrictX)
      range(rnd.time)
    else
      range(c(rnd.time, prx.time)),
    ylim = range(c(rnd.mean, prx.mu, unlist(
      lapply(rnd.match, function(i) {
        mu[[i]]
      })
    ))),
    type = "n",
  ...
  if (restrictX) {
    ...
    lines(
      lowess(prx.time, prx.mu),
      lty = 1,
      lwd = 2,
      col = 1
    )
  }
  legend("bottomleft", bty = "n", legend = c(paste("avg prx.", format(
    mean(rnd.prx, na.rm = TRUE), digits = 3
  ))))
